    ...
    var panZoomFunctions = {
        ...
        PanZoom = function (el, options) {
            ...
            function repaint() {
                ...
                var newWidth = width() * (1 - (me.currZoom * settings.zoomStep)),
                    newHeight = height() * (1 - (me.currZoom * settings.zoomStep));

                if (me.currPos.x < 0) {
                    me.currPos.x = 0;
                } else if (me.currPos.x > (width() * me.currZoom * settings.zoomStep)) {
                    me.currPos.x = (width() * me.currZoom * settings.zoomStep);
                }

                if (me.currPos.y < 0) {
                    me.currPos.y = 0;
                } else if (me.currPos.y > (height() * me.currZoom * settings.zoomStep)) {
                    me.currPos.y = (height() * me.currZoom * settings.zoomStep);
                }
            ...
            function dragging(e) {
                ...
                var evt = window.event || e,
                    newWidth = width() * (1 - (me.currZoom * settings.zoomStep)),
                    newHeight = height() * (1 - (me.currZoom * settings.zoomStep)),
                    newPoint = getRelativePosition(evt, container);

                deltaX = (newWidth * (newPoint.x - initialPos.x) / width()) * -1;
                deltaY = (newHeight * (newPoint.y - initialPos.y) / height()) * -1;
                initialPos = newPoint;
            ...
            function applyZoom(val, centerPoint) {
                ...
                if (me.currZoom < settings.minZoom) {
                    ...
                    centerPoint = centerPoint || { x: width() / 2, y: height() / 2 };

                    deltaX = ((width() * settings.zoomStep) * (centerPoint.x / width())) * val;
                    deltaY = (height() * settings.zoomStep) * (centerPoint.y / height()) * val;
